...
import { listSaved, type Article } from "@/services/articles";
...
const Saved: React.FC = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <h1 className="text-2xl font-semibold">Enregistrés</h1>
      {loading && <div className="mt-4">Chargement…</div>}
      {err && <div className="mt-4 text-red-600">{err}</div>}

      <div className="mt-4 space-y-3">
        {items.length === 0 && !loading ? (
          <div className="text-neutral-600">Aucun article enregistré.</div>
        ) : (
          items.map((a) => (
            <Link
              to={`/articles/${a.id}`}

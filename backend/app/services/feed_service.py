...
class FeedService:
    ...
    def create(
        ...
        *,
        ...
        if c.owner_id != created_by:
            m = self.members.get_membership(collection_id=collection_id, user_id=created_by)
        ...
        if self.feeds.get_by_url_in_collection(collection_id=collection_id, url=url_n):
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Feed URL already exists in this collection")
        ...
        if names:
            ids = self.categories.ids_by_names(names)  # à implémenter dans CategoryRepository
            if ids:
    ...
    def update(self, feed_id: int, **data) -> Feed:
        ...
        if url is not None:
            ...
            dup = self.feeds.get_by_url_in_collection(collection_id=feed.collection_id, url=url_n)

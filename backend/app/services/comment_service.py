...
class CommentService:
    ...
    def _ensure_parent_in_same_article(self, article_id: int, parent_id: Optional[int]) -> Tuple[Optional[Comment], int]:
        if not parent_id:
            return None, 0
    ...
    def create(self, *, article_id: int, author_id: int, content: str, parent_id: Optional[int]) -> Comment:
        self._ensure_article(article_id)
        ...
        self.db.execute(
            update(ArticleModel).where(ArticleModel.id == article_id).values(
                total_comments=ArticleModel.total_comments + 1
            )
        )
        self.db.commit()
        self.db.refresh(c)
    ...
    def update(self, *, comment_id: int, author_id: int, content: str, is_admin: bool) -> Comment:
    ...
    def vote(self, *, comment_id: int, author_id: int, kind: str, delta: int) -> Comment:

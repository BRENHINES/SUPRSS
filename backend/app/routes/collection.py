...
from ..schemas.collection import (CollectionCreate, CollectionOut,
                                  CollectionUpdate, MemberAdd, MemberOut,
                                  MemberUpdate)
...
def list_my_collections(
    ...
    items, total = CollectionRepository(db).list_for_user(
        user_id=user.id, q=q, page=page, size=size
    )
...
@router.get(
    "/all", response_model=list[CollectionOut], dependencies=[Depends(require_admin)]
)
...
def get_collection(
    collection_id: int, db: Session = Depends(get_db), user=Depends(get_current_user)
):
    ...
    if not c.is_public:
        # vérifie membership
        m = CollectionMemberRepository(db).get_membership(
...
def list_members(
    ...
    items, total = CollectionMemberRepository(db).list_for_collection(
...
def add_member(
    ...
    m = CollectionService(db).add_member(
        collection_id=collection_id, actor_id=user.id, **payload
    )
    return m
...
def update_member(
    ...
    m = CollectionService(db).update_member(
        collection_id=collection_id,
        member_id=member_id,
        actor_id=user.id,
        **data.model_dump(exclude_unset=True)
...
def remove_member(
    ...
    CollectionService(db).remove_member(
        collection_id=collection_id, member_id=member_id, actor_id=user.id
    )
    return

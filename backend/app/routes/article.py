...
from ..schemas.article import ArticleOut, ArticleWithState, ArticleUserState, InteractionUpdate
...
def list_feed_articles(
    ...
    items, _total = ArticleService(db).list_feed_articles(feed_id=feed_id, q=q, from_date=from_date, page=page, size=size)
    return items

@router.get("/articles/{article_id}", response_model=ArticleOut)
def get_article(article_id: int, db: Session = Depends(get_db), user = Depends(get_current_user)):
...
def list_articles(
    ...
    base = (
        ...
        .join(CollectionMember, CollectionMember.collection_id == Collection.id, isouter=True)
    ...
    base = _apply_article_filters(base, q=q, language=language, date_from=date_from, date_to=date_to)

    # filtres user-specific (UserArticle)
    if unread_only or favorite_only or archived_only:
        base = base.join(UserArticle, UserArticle.article_id == Article.id, isouter=True)\
                   .where(UserArticle.user_id == user.id)
        if unread_only:
            base = base.where(or_(UserArticle.is_read == False, UserArticle.is_read.is_(None)))
        if favorite_only:
    ...
    items = db.scalars(_apply_sort(base, sort)
                       .offset((page - 1) * size)
                       .limit(size)).all()

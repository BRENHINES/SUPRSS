...
def fetch_one_feed(
    ...
    _user=Depends(
        get_current_user
    ),  # si besoin, vérifie droits sur la collection du feed
):
...
@router.post(
    "/collections/{collection_id}/feeds/fetch",
    summary="Fetch tous les feeds d'une collection (séquentiel simple)",
)
def fetch_collection(
    ...
    feeds = FeedRepository(db).list_by_collection(
        collection_id=collection_id, filters=None, pagination=None
    )

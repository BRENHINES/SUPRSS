...
from ..schemas.feed import FeedCreate, FeedUpdate, FeedOut, FeedAttachCategories
...
@router.post("/collections/{collection_id}/feeds", response_model=FeedOut, status_code=201)
def create_feed(
    collection_id: int,
    data: FeedCreate,
...
def update_feed(feed_id: int, data: FeedUpdate, db: Session = Depends(get_db), _=Depends(get_current_user)):
...
def delete_feed(feed_id: int, db: Session = Depends(get_db), _=Depends(get_current_user)):
    FeedService(db).delete(feed_id)
    return

@router.post("/feeds/{feed_id}/categories", status_code=204)
def attach_categories(feed_id: int, data: FeedAttachCategories, db: Session = Depends(get_db), _=Depends(get_current_user)):
...
def detach_category(feed_id: int, category_id: int, db: Session = Depends(get_db), _=Depends(get_current_user)):
